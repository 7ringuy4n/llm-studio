...
class ChatMessage(BaseModel):
    ...
    def validate_content(self) -> "ChatMessage":
        if isinstance(self.content, str):
            if not self.content or len(self.content) > 32_768:
                raise ValueError("text content must contain 1-32768 characters")
            return self
        ...
        for message in self.messages:
            ...
            for part in message.content:
                if isinstance(part, TextContentPart):
                    ...
                    image_count += 1
        if text_bytes > 60_000:
            raise ValueError("combined text content is too large")
        if image_count > 2:
